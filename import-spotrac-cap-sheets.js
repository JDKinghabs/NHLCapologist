...
const DATA_PATH = 'C:/Scripts/NHL/Capologist/data/nhl-cap-data.json';
const SEASON = '2025-26';
const YEAR = 2025;
const FETCH_DATE = '2026-04-19';
...
function fetchSpotracHtml(teamSource) {
...
function ensurePlayer(data, playerIndex, row, teamAbbr) {
...
function ensureSeasonContract(data, playerId, teamAbbr, row) {
  ...
  let contract = (data.contracts || []).find((entry) =>
    entry.playerId === playerId &&
    entry.team === teamAbbr &&
    isActiveSeason(entry, SEASON, seasons)
  );
...
function buildCapSheet(teamSource, parsed, data, playerIndex) {
  ...
  parsed.active.forEach((row) => {
    const playerId = ensurePlayer(data, playerIndex, row, teamSource.abbr);
...
function main() {
  ...
  data.meta.notes = `2025-26 cap sheets imported from Spotrac on ${FETCH_DATE}. Future cap figures remain projections.`;
  ...
  sources.forEach((teamSource) => {
    const html = fetchSpotracHtml(teamSource);
  ...
  console.log(`Imported Spotrac cap sheets for ${report.length} teams (${SEASON}).`);
  report.forEach((row) => {
    console.log(`${row.abbr}: active=${row.active}, minor=${row.minor}, buyout=${row.buyout}, retained=${row.retained}`);
  });
